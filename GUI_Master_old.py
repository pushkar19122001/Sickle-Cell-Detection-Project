...
#
lbl = tk.Label(root, text="Sickle Cell Anemia Detection", font=('times', 35,' bold '), width=65,height=1,bg="#152238",fg="white")
lbl.place(x=0, y=0)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        model = load_model('cell_model.h5')
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='C:/Code/Code/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
